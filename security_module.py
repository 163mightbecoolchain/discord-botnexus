...
class _SlashCtx:
    """
    Мостик Interaction → ctx-подобный объект.
    Диспетчер /q написан под commands.Context и использует
    ctx.send / ctx.guild / ctx.author / ctx.message.mentions —
    здесь эти четыре вещи и эмулируются.
    """
    def __init__(self, interaction: discord.Interaction):
        ...
        self.message = _SlashMsg(interaction)

    async def send(self, *a, **kw):
        kw.pop("delete_after", None)   # у followup такого параметра нет
        return await self.interaction.followup.send(*a, **kw)


class _SlashMsg:
    """Заглушка ctx.message: упоминания берём из resolved-данных слэш-команды"""
    def __init__(self, interaction: discord.Interaction):
        self._it = interaction

    @property
    def mentions(self):
        data = self._it.data or {}
        resolved = data.get("resolved") or {}
        ids = list((resolved.get("members") or {}).keys())
        ids += [u for u in (resolved.get("users") or {}).keys() if u not in ids]
        out, g = [], self._it.guild
        for uid in ids:
            try:
...
class AdvancedSecurityCog(commands.Cog, name="AdvancedSecurity"):
    ...
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    #  КОМАНДА /q  (раньше была префиксной -q)
    #  Префикс-команды убраны: Discord требует Message Content Intent
    ...
    async def q_slash(self, interaction: discord.Interaction,
                      subcmd: str = "help", target: str = "", extra: str = ""):
        """Слэш-версия: собирает shim-контекст и вызывает общий диспетчер"""
